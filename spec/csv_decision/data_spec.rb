...
describe CSVDecision::Data do
  it 'parses an CSV string' do
    result = CSVDecision::Data.to_array('')
  ...
  it 'parses a CSV file' do
    file = Pathname(File.join(CSVDecision.root, 'spec', 'data', 'empty.csv'))
    result = CSVDecision::Data.to_array(file)
    expect(result).to be_a Array
    expect(result.empty?).to eq true

    file = Pathname(File.join(CSVDecision.root, 'spec', 'data', 'valid.csv'))
    result = CSVDecision::Data.to_array(file)
    expect(result).to be_a Array
    expect(result).to eq [['IN :input', 'OUT :output'], ['input', '']]
  end
  ...
  it 'raises an error for invalid input' do
    expect { CSVDecision::Data.to_array({}) }
